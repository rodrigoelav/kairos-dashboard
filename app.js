/* ==========================================================================
   KAIRÓS MOTORES - DASHBOARD ENGINE (VANILLA JS ES6)
   ========================================================================== */

// 1. DADOS INICIAIS DA IMAGEM DE REFERÊNCIA
...
let bgConfig = JSON.parse(localStorage.getItem('kairos_bg_config')) || {
  type: 'canvas', // 'canvas', 'video', 'gif'
  videoUrl: 'https://assets.mixkit.co/videos/preview/mixkit-circuit-board-loop-animation-43098-large.mp4',
  gifUrl: '',
  opacity: 70,
  blur: 2,
  canvasSpeed: 2
...
// 5. RENDERIZAÇÃO DOS CARTÕES E MÉTRICAS
  ...
  filtered.forEach(project => {
    ...
    card.innerHTML = `
      ...
      <div class="card-content">
        ...
        <p class="card-responsibles">(${escapeHtml(project.responsibles)})</p>
      </div>
      <div class="card-footer">
        <span class="status-badge ${statusInfo.class}" title="Clique para alternar o status rápido">${escapeHtml(project.status)}</span>
      </div>
      <button class="card-edit-btn" title="Editar Projeto">
    ...
    // Evento para alternar status diretamente ao clicar no badge
    ...
    // Evento de abrir modal ao clicar no cartão
...
// 6. GERENCIAMENTO DE MODAIS (CRIAR E EDITAR)
...
function openEditModal(project) {
  ...
  document.getElementById('project-icon-input').value = project.icon || 'api';
  document.getElementById('project-progress-input').value = project.progress || 50;
...
formProject.addEventListener('submit', (e) => {
  ...
  if (id) {
    ...
    if (proj) {
      proj.title = title;
      proj.responsibles = responsibles;
      proj.status = status;
      proj.icon = icon;
...
// 7. GERENCIAMENTO DE FUNDO DINÂMICO (CANVAS / VÍDEO / GIF)
...
function applyBackgroundConfig() {
  // Ajusta desfoque e transparência
  ...
  if (bgConfig.type === 'video' && bgConfig.videoUrl) {
    bgVideo.src = bgConfig.videoUrl;
    bgVideo.play().catch(() => {});
...
// Renderizador Canvas de Partículas Tech & Engrenagens Futuristas
...
function animateCanvas() {
  if (bgConfig.type === 'canvas') {
    ...
    // Desenhar engrenagens de fundo
    canvasAngle += 0.002 * (bgConfig.canvasSpeed || 2);
    gears.forEach(g => {
      drawGear(canvasCtx, g.x, g.y, g.radius, g.teeth, canvasAngle * (g.speed > 0 ? 1 : -1));
    });

    // Desenhar partículas conectadas
...
// Eventos do Modal de Fundo Dinâmico
...
document.querySelectorAll('.bg-type-btn').forEach(btn => {
  btn.addEventListener('click', (e) => {
    bgConfig.type = e.target.getAttribute('data-type');
    updateBgSelectorUI();
...
function updateBgSelectorUI() {
  document.querySelectorAll('.bg-type-btn').forEach(b => {
    b.classList.toggle('active', b.getAttribute('data-type') === bgConfig.type);
  });
...
document.getElementById('bg-video-url').addEventListener('change', (e) => {
  bgConfig.videoUrl = e.target.value.trim();
  applyBackgroundConfig();
  saveState();
});

document.getElementById('bg-gif-url').addEventListener('change', (e) => {
  bgConfig.gifUrl = e.target.value.trim();
  applyBackgroundConfig();
  saveState();
});
...
document.getElementById('btn-more-options').addEventListener('click', () => {
  ...
  document.getElementById('tv-share-url').value = tvUrl;
});
...
document.getElementById('btn-copy-tv-url').addEventListener('click', () => {
  ...
  alert('Link do Modo TV copiado para a área de transferência!');
});
...
document.getElementById('btn-export-data').addEventListener('click', () => {
  ...
  downloadAnchor.setAttribute("download", `kairos_projetos_${new Date().toISOString().slice(0,10)}.json`);
...
document.getElementById('btn-reset-defaults').addEventListener('click', () => {
  if (confirm('Deseja restaurar a lista inicial dos 10 projetos da Kairós Motores?')) {
...
// 10. EVENTOS DOS FILTROS E BUSCA
